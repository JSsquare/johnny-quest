...
- Recommendations are generated by an OpenAI Responses API endpoint at `app/api/ask/route.ts` which streams text back to the client.
...
- Backend / API
  - OpenAI streaming route (Node runtime): `app/api/ask/route.ts`
    - Uses the OpenAI Responses API and streams text back to the client.
...
- The OpenAI route uses a Node runtime and calls the OpenAI Responses API directly; review `app/api/ask/route.ts` for the request and streaming implementation.
